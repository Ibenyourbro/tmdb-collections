...
async function getCatalogResponse(req) {
  ...
  if (extra.search) {
    ...
    switch (req.params.id) {
      ...
      case `tmdbcf${dev}.disneyPrincess`:
        // Use specific list of Disney Princess movies and collections
        return await cacheWrapCatalog(cacheKey, async () => {
          console.log('Processing Disney Princess catalog');
          
          // First, get collections from our predefined list
          let collections = [...disneyPrincess.collections];
          
          // Then get collections from our predefined movies
          const movieCollections = await getCollectionIDsFromMovieIDs(disneyPrincess.movies);
          collections = [...new Set([...collections, ...movieCollections])];
          
          // Process collection details
          const metas = await processCollectionDetails(collections, 'popularity');
          console.log(`Found ${metas.length} Disney Princess collections`);
          return { metas };
        });
        break;
